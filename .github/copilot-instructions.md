...
- Preserve the official-vs-pending accounting model. `Total` combines locally observed assistant/compaction usage while the official counter lags, then statusline `ai_used.total_nano_aiu` reconciles the durable total and subtracts caught-up pending cost. `Sess` displays that same official SDK session counter. 24h/7d/30d windows are derived from `session-ledger.json`: statusline updates the current `session_id`, startup sync imports whitelisted cost/token metadata from new/null JSONL sessions, and stale open sessions older than 7 days become `auto_closed`. Usage-based billing started on 2026-06-01, so pre-cutover pay-per-message totals must not be treated as usage-based cost; use retained token telemetry to estimate historical equivalent cost under the current usage-based model when a post-cutover token-rate profile exists. Do not replace this with local-only `assistant.usage` totals, do not re-aggregate known `open`/`closed`/`auto_closed` sessions from JSONL, and keep the `officialStartedUsd` turn-start guard to avoid double-counting. If using account/quota APIs, avoid flows that unexpectedly trigger OS keychain prompts; prefer already-authenticated extension surfaces or explicit opt-in.
